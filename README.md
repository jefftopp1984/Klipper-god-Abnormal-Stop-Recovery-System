...
********* NOTE: If Klipper fails to run the _SAVE_FILE macro because it is busy handling other start-of-print tasks, simply move the _SAVE_FILE macro call to a LATER point in the start print script. There could also be some strange cases where Klipper will not want to run the macro the first time around. If this happens, create a new delayed_gcode named RESAVE_FILE with the command within that being _SAVE_FILE and then insert the line UPDATE_DELAYED_CODE ID=RESAVE_FILE DURATION=2 a few lines after the initial _SAVE_FILE macro call. For some reason, some printers have a problem with some configuration structures if there are several [include *.cfg] lines, especially ones that are doubled like the first included file includes a second file or even a third file. A workaround to solve this is to call the macro a second time with delayed gcode. This is a strange edge-case that is rare, but possible. ***************
